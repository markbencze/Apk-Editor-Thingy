...
echo  _______________________________________________________________________________
echo  			  Apk Editor Alpha release
echo  _______________________________________________________________________________
echo  		       These options are rarely used
echo  _______________________________________________________________________________
echo  0    Adb pull
echo  1    Extract apk
...
echo  			       Main options
echo  _______________________________________________________________________________
echo  9    Decompile app
echo  10   Decompile apk with dependencies
echo  11   Compile apk
echo  12   Sign apk 
echo  13   Install apk
echo  14   Compile apk / Sign apk / Install apk 
echo  _______________________________________________________________________________
echo 		    		Other stuff
echo  _______________________________________________________________________________
echo  15   Batch Optimize apk 
echo  16   Sign an apk
echo  17   Batch optimize ogg files 
echo  18   Clean Files/Folders
echo  19   Select compression level 
echo  20   Set Max Memory Size
echo  21   Read the log because you screwed up!
...
:WHAT
echo Your entry was unrecognized, Try again
PAUSE
goto restart
:cleanp
...
echo are doing wrong, then post in the thread on xda with proper information
...
java -Xmx%heapy%m -jar apktool.jar d -q ../place-apk-here-for-modding/%capp% -o ../projects/%capp%
if errorlevel 1 (
echo "An Error Occured, Please Check The Log (option 21)"
PAUSE
)
...
echo Building Apk
IF EXIST "%~dp0place-apk-here-for-modding\unsigned%capp%" (del /Q "%~dp0place-apk-here-for-modding\unsigned%capp%")
java -Xmx%heapy%m -jar apktool.jar b -q "../projects/%capp%" -o "%~dp0place-apk-here-for-modding\unsigned%capp%"
if errorlevel 1 (
echo "An Error Occured, Please Check The Log (option 21)"
PAUSE
)
...
if %INPU%==n (goto q1)
REM :nq1
REM echo Aside from the signatures, would you like to copy
REM echo over any additional files that you didn't modify
REM echo from the original apk in order to ensure least 
REM echo # of errors ^(y/n^)
REM set /P INPUT1=Type input: %=%
REM if %INPUT1%==y (call :nq2)
REM if %INPUT1%==n (call :nq3)
REM :nq2
REM rmdir /S /Q "%~dp0keep"
REM 7za x -o"../keep" "../place-apk-here-for-modding/%capp%"
REM echo In the apk editor folder you will find
REM echo a keep folder. Within it, delete 
REM echo everything you have modified and leave
REM echo files that you haven't. If you have modified
REM echo any xml, then delete resources.arsc from that 
REM echo folder as well. Once done then press enter 
REM echo on this script.
REM PAUSE
REM 7za a -tzip "../place-apk-here-for-modding/unsigned%capp%" "../keep/*" -mx%usrc% -r
REM rmdir /S /Q "%~dp0keep"
REM cd ..
REM goto restart
REM :nq3
